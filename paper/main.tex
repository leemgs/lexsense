...
%   colorlinks=true + urlcolor=blue makes every reference URL print as
%   BLUE clickable text, so the author can click each link in the
%   References section and verify that the cited paper actually exists
%   before submission. Internal links and citations stay black.
%   For the camera-ready version, set colorlinks=false (or urlcolor=black)
%   and/or remove the `url` fields in reference-data.bib.
\hypersetup{
  colorlinks   = true,  %Colour link text (needed so urlcolor takes effect)
  urlcolor     = blue,  %Colour for external hyperlinks (reference URLs)
